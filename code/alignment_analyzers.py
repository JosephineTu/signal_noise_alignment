...
class IBLAlignmentBase:
    ...
    def decode_bin_foldwise(
        ...
        for fold in folds_local:
            ...
            w_list.append(w)
        w_mean = np.mean(w_list, axis=0) if w_list else np.array([np.nan, np.nan])
        w_norm = np.linalg.norm(w_mean)
        w_mean = w_mean / (w_norm + self.cfg.eps) if w_norm > self.cfg.eps else w_mean
        return {
            "w_mean": w_mean,
...
class TimeResolvedAlignmentAnalyzer(IBLAlignmentBase):
    ...
    def make_time_windows(t_start, t_end, bin_size, step_size):
        """
        Create time bin windows from t_start to t_end with given bin_size and step_size.
        """
        starts = np.arange(t_start, t_end - bin_size + 1e-12, step_size)
        ends = starts + bin_size

        if len(starts) < 1:
            raise ValueError("Time range too small for given bin_size.")
        windows = np.c_[starts, ends]
        return windows

    def trial_binned_firing_rates(self, spikes, region_cluster_ids, stim_on, windows):
        """
    ...
    def run_one_eid(self, eid: str, do_plots=True) -> Dict[str, Any]:
        ...
        windows = self.make_time_windows(self.t_start, self.t_end, 
                                         self.bin_size, self.step_size)
        ...
        if do_plots:
            ...
            plt.figure(figsize=(7,4))
            plt.plot(times, noise_subspace_dim_ts, marker='o', label='noise subspace dim')
            plt.xlabel("time from stimOn (s)")
            plt.ylabel("dimension")
            plt.title("Time-resolved noise subspace dimension")
            plt.legend(frameon=False)
            plt.tight_layout()
            plt.savefig(f"plots/{eid}_noise_subspace_dim_ts.png", dpi=300, bbox_inches="tight")
            plt.close()
            ...
            if self.do_decode:
                ...
                plt.plot(times, cos_w_sig, marker="o", label="cos(w, signal)")
                plt.plot(times, cos_w_noi, marker="o", label="cos(w, noise)")

                plt.ylim(0, 1.05)
                plt.xlabel("time from stimOn (s)")
                plt.ylabel("alignment")
                plt.title("Decoder geometry")
                ...
                plt.savefig(f"plots/{eid}_decoder_geometry.png", dpi=300, bbox_inches="tight")
                plt.close()
...
if __name__ == "__main__":
    ...
    # choose which eid this SLURM task should run
    ...
    out_dir = Path("results_time_resolved")
    ...
    try:
        ...
        with open(out_dir / f"{eid}.pkl", "wb") as f:

            pickle.dump(out, f)

        print(f"saved result to {out_dir / f'{eid}.pkl'}")
